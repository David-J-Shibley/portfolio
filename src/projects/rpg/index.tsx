...
const RPGGame: React.FC = () => {
  ...
  const [message, setMessage] = useState<string>(
    "Welcome to the TypeScript RPG! Create your character to begin."
  );
  ...
  const shopItems: Item[] = [
    ...
    },
    {
      ...
      name: "Orb of Chaos",
      description:
        "Grants immense power but unpredictable effects (+20 attack, +10 defense, -10 max health)",
      cost: 250,
  ...
  const handleClassChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const classStats: Record<
      CharacterClass,
      { maxHealth: number; attack: number; defense: number }
    > = {
      warrior: { maxHealth: 120, attack: 12, defense: 8 },
      rogue: { maxHealth: 80, attack: 15, defense: 4 },
      mage: { maxHealth: 70, attack: 18, defense: 2 },
      paladin: { maxHealth: 110, attack: 10, defense: 10 },
      berserker: { maxHealth: 100, attack: 20, defense: 3 },
      ranger: { maxHealth: 85, attack: 14, defense: 6 },
      necromancer: { maxHealth: 60, attack: 20, defense: 3 },
      monk: { maxHealth: 95, attack: 13, defense: 9 },
      assassin: { maxHealth: 75, attack: 22, defense: 2 },
      summoner: { maxHealth: 65, attack: 10, defense: 5 },
    };

    if (classStats[characterClass]) {
      stats = {
        ...stats,
        ...classStats[characterClass],
        health: classStats[characterClass].maxHealth,
      };
    }
  ...
  const startGame = () => {
    ...
    setMessage(
      `${character.name} the ${character.class} begins their adventure!`
    );
    setGameState("exploration");
  };
  ...
  const startCombat = () => {
    ...
    while (enemyIndex > enemies.length - 1 || enemyIndex <= 0) {
      enemyIndex = Math.floor(
        ((Math.random() * enemies.length) / 2) * character.level
      );
    }
  ...
  const rest = () => {
    ...
    setMessage(
      `You take a short rest and recover ${
  ...
  const attack = () => {
    ...
    const damage = Math.max(
      Math.floor(
        character.attack * (Math.random() * 0.5 + 0.75) - currentEnemy.defense
      ),
      1
    );
    ...
    const enemyDamage = Math.max(
      Math.floor(
        currentEnemy.attack * (Math.random() * 0.5 + 0.75) - character.defense
      ),
      1
    );
  ...
  const flee = () => {
    ...
    if (fleeChance < 0.6) {
      ...
      if (currentEnemy) {
        const enemyDamage = Math.max(
          Math.floor(
            currentEnemy.attack * (Math.random() * 0.5 + 0.75) -
              character.defense
          ),
          1
        );
        ...
        setMessage(
          `You failed to flee! The ${currentEnemy.name} hits you for ${enemyDamage} damage!`
        );
  ...
  const endCombat = (victory: boolean) => {
    if (victory && currentEnemy) {
      ...
      let updatedCharacter = {
        ...character,
        experience: levelUp
          ? newExperience - 100 * character.level
          : newExperience,
      ...
      if (levelUp) {
        ...
        setMessage(
          `Victory! You defeated the ${currentEnemy.name} and gained ${experienceReward} experience and ${goldReward} gold! You leveled up to level ${updatedCharacter.level}!`
        );
      } else {
        setMessage(
          `Victory! You defeated the ${currentEnemy.name} and gained ${experienceReward} experience and ${goldReward} gold!`
        );
      }
  ...
  const buyItem = (item: Item) => {
    ...
    if (item.id === "health-potion") {
      ...
      setMessage(
        `You used a Health Potion and recovered health! (${character.health} → ${newHealth})`
      );
      return;
    }
    ...
    setMessage(
      `You purchased ${item.name} for ${item.cost} gold! Your stats have been updated.`
    );
    ...
    <div className="max-w-lg mx-auto p-4 bg-gray-100 rounded-lg shadow-lg">
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="mb-4">
            ...
            >
              ...
              <option value="berserker">
                Berserker (Very High Attack, Low Defense)
              </option>
              <option value="ranger">
                Ranger (Balanced Attack, Ranged Combat)
              </option>
              <option value="necromancer">
                Necromancer (Summons Minions, Dark Magic)
              </option>
              <option value="monk">
                Monk (Balanced, High Evasion)
              </option>
              <option value="assassin">
                Assassin (Stealthy, Critical Hits)
              </option>
              <option value="summoner">
      ...
      {gameState === "exploration" && (
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <p>
                EXP: {character.experience}/{100 * character.level}
              </p>
                ...
                <button
                  ...
                  disabled={
                    ...
                    (item.id !== "health-potion" &&
                      character.inventory.some((i) => i.id === item.id))
                  }
                >
                  Buy
                </button>
      ...
      {gameState === "gameOver" && (
        <div className="bg-white p-4 rounded shadow text-center">
          ...
          <p>
            You reached level {character.level} and collected {character.gold}{" "}
            gold.
          </p>
